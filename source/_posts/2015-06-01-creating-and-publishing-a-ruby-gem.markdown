...
Source code slug_converter gem was developed while working on a link shortener application, in order to generate a string consisting of predefined characters, based on a given id number of a link. As it will be described in this  blog post, this code was easily extracted from the application and into an independent gem that was released on  RubyGems.
...
Answering these questions will help bundler configure and setup files that are being generated now and in the future. Here we answered yes to first 3 qestions and choose rspec for testing.

Running this command resulted with "slug_converter" directory with essential gem file structure being created, and git repository initialized, assuming that you are using git for version management (as you should).
...
Let's go through files that bundler generated for us, .gemspec file is the "heart" of your gem so lets start with `slug_converter.gemspec`
	...
	Gem::Specification.new do |spec|
	  ...
	  spec.authors      = ["Your Name"] //these ware populated by bundler 
	  spec.email        = ["youremail@example.com"] //these ware populated by bundler 
...
A very important part of every gem is the README file, where you can describe how to install and use your gem, and the LICENCE file where you can define the terms and conditions under which your gem can be used.
...
It was mentioned in the beginning that, when running bundle gem for the first time bundler will asks if you would like to generate test files for your gem and to choose if you want to use rspec or minitest. If you answer with yes, and choose rspec, bundler will generate a spec directory with two files: 

	|-- spec
    	  |-- slug_converter_spec.rb
		  |-- spec_helper.rb

In the spec_helper file you can reference any test globals or configuration.
...
**Now we need to make those test go green!**

To do that we will again copy the existing code from our application in the main gem file `lib/slug_converter.rb`:
...
Now run those tests again, and watch them all pass.
